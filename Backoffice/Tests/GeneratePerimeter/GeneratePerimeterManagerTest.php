...
class GeneratePerimeterManagerTest extends AbstractBaseTestCase
{
    ...
    public function testGeneratePerimeters()
    {
        ...
        Phake::when($strategy0)->generatePerimeter()->thenReturn($perimeter0);
        Phake::when($strategy0)->getPerimeterConfiguration()->thenReturn($perimeter0);
        ...
        Phake::when($strategy1)->generatePerimeter()->thenReturn($perimeter1);
        Phake::when($strategy1)->getPerimeterConfiguration()->thenReturn($perimeter1);
        $this->manager->addStrategy($strategy1);

        $this->assertEquals(array($key0 => $perimeter0, $key1 => $perimeter1), $this->manager->generatePerimeters());
        $this->assertEquals(array($key0 => $perimeter0, $key1 => $perimeter1), $this->manager->getPerimetersConfiguration());
    }
}
